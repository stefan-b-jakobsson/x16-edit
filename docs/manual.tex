...
\section{File handling}
    ...
    The file name to read or write may be typed in directly at the prompt.

    Files may alternatively be selected in a file browser view that you enter by pressing Ctrl+T. 
    In the file browser view, you highlight items by pressing the up or down arrow keys.
    The highlighted file is selected by pressing Enter. If the highlighted item is a directory,
    it will be made the new current directory. If all items in the current directory fits on the screen, 
    the listing is ended with "--- END ---".
    If there are more items, the program prints "--- MORE ---" instead. In case
    the items are spread over several pages, you may go to next page with Ctrl+V and back to a 
    previous page with Ctrl+Y.

    By default the program uses device \#8. The device
    number may be changed by pressing Ctrl+D.

    It is possible to send commands to the disk by pressing Ctrl+I. You may type any valid
    CBM DOS command at the prompt. As an example, "S:filename" deletes ("scratches") a file.
    Be careful, as there is nothing stopping you from deleting files or even formatting the
    disk. You find a list of available commands at 
    \href{https://github.com/commanderx16/x16-rom/tree/master/dos}{github.com/commanderx16/x16-rom/tree/master/dos}.
