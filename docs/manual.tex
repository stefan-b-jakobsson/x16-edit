...
\section{User-configurable command key bindings}
\label{keybindings}

It is possible to redefine the command key bindings in X16 Edit. 

On startup the program reads the file X16EDITRC in the root folder of the SD card, if present. 
That file should contain a stream of bytes identifying the key bindings. The key bindings are 
sorted in the same order as the commands are listed in section \ref{commands}.

To make setting up the key bindings a bit easier, there is a special configuration tool (X16EDIT-KEYBINDINGS.PRG).
